...
:::{note}
The IOC will only restart on a configuration change _if_ the macros provided for the `tpar` files are different, otherwise the IOC (and any changes to the TPAR file) will remain. To work around this and load the master file's contents, simply press the "reset to original" button in the device screen or restart the IOC manually.
:::
