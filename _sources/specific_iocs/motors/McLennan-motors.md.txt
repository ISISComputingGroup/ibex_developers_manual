...
### Motor Resolution
The motor resolution is set with the *MSTP***n** IOC macros and in units of `steps/mm`, this will be inverted by the IOC internally as the EPICS motor record `MRES` field uses the inverse `mm/step`. 
...
### Limits
There are a pair of macros for the limits *DHLM***n** and *DLLM***n** for the high and low limit respectively.
