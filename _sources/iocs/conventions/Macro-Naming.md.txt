...
| `LVDCOM_HOST` | The host of the LVDcom vi (see [Creating IOC wrapper VI](../creation/Creating-IOC-wrapper-VI) for more). | "" | No |
| `LVDCOM_OPTIONS` | LVDcom options for starting the vi (see [Creating IOC wrapper VI](../creation/Creating-IOC-wrapper-VI) for more). | 6 | No |
| `LVDCOM_USER` | LVDcom user; not to be added to `config.xml`. It can be set only through the local globals.txt (see [Creating IOC wrapper VI](../creation/Creating-IOC-wrapper-VI) for more). | "" | No |
| `LVDCOM_PASS` | LVDcom passphrase; not to be added to `config.xml`. It can be set only through the local globals.txt (see [Creating IOC wrapper VI](../creation/Creating-IOC-wrapper-VI) for more). | "" | No |
