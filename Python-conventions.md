...
**We have a [Ruff config](https://github.com/ISISComputingGroup/reusable-workflows/blob/main/ruff.toml), you can setup ruff to work with most IDE's and then use our [config](https://github.com/ISISComputingGroup/reusable-workflows/blob/main/ruff.toml) to lint your code, or run ruff from command line before making a pull request, standards compliance will be enforced by this config at pull request. So before making a pull request you should ensure any files you have changed have been formatted by ruff, and pass its checks. Ruff can also automatically fix many violations.**
