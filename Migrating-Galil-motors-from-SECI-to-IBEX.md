...
This last line can be made clearer by setting each of the homing modes as an environment variable, and referencing these in the call to `GalilStartController`. An example of this can be found in `\\Ndxloq\c$\Instrument\Settings\config\NDXLOQ\configurations\galil\galil01.cmd`:
