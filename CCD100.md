...
Set `IPADDR` macro to IP address of unit. Pressure section have been allocated a range of IP addresses to used for these devices, so we should not need to assign one. Check that the subnet mask is 255.255.**252**.0 and not 255.255.255.0  The units do respond to a ping when correctly connected to the network.