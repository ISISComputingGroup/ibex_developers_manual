...
For most instruments `$(PERIOD)` will be `1`, however some instruments scan their instruments and detector/monitor data at each scan point can be accessed via using a different value of `$(PERIOD)`. SANS, reflectometry and muon instruments regularly use multiple periods, most other instruments only occasionally if at all.
