...
* Updating the clean ibex script. The script to clean the server is stored in `RCPTT_Tests\cleanIBEXServer\cleanIBEXServer.py` but this is not run directly from the system tests. Instead the `CleanIBEXServerFiles` context copied it to a known location. To update the context remove the current folder and then add in the cleanIBEXServer folder. This updates the file.
